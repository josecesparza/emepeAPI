	...
	public ArrayList<Producto> getProductos(){
		...
		try {
			...
			while (rs1.next()) {
				...
				//Mapa<id Opinion, id OpinionAnterior>;
				HashMap<Integer, Integer> opiniones = new HashMap<Integer, Integer>();
				while(rs4.next()) {
					if(rs4.getObject(4) == null) {
						opiniones.put(rs4.getInt(2), null);
					}
					else {
						opiniones.put(rs4.getInt(2), rs4.getInt(4));
					}
				}
				//Recorrer HashMap i recrear Opiniones a partir de la null;
